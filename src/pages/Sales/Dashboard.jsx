  ...
  return (
    <>
      ...
      <Container maxWidth="lg" sx={{ py: 5 }} >
        ...
        <Box display="flex" flexWrap="wrap" justifyContent="right" gap={2} mb={3}>
        ...
        <Grid item xs={12} sm={6} md={4} lg={3} key={delivery._id}>
          <Card elevation={3} sx={{ borderRadius: 4, p: 2 }}>
            <CardContent>
              ...
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                ...
                <Chip
                  ...
                  color={
                    ...
                    delivery.status === 'Heading to Customer' ? 'primary' :
                    delivery.status === 'Waiting for Paperwork' ? 'warning' :
                    delivery.status === 'In Route for Pick Up' ? 'info' :
                    'default'
                  }
                />
              </Box>
              ...
              {/* Vehicle */}
              <Typography variant="body2" color="text.secondary" fontWeight={500}>
                Vehicle: {delivery.year} {delivery.make} {delivery.model} {delivery.trim} - {delivery.color}
              </Typography>
              ...
              >
                COD: ${delivery.codAmount} {delivery.codCollected ? `(via ${delivery.codMethod})` : '(Pending)'}
              </Typography>

              {/* Lease Return */}
              <Chip
                label={delivery.leaseReturn?.willReturn ? 'Lease Return' : 'No Lease Return'}
                color={delivery.leaseReturn?.willReturn ? 'warning' : 'default'}
                size="small"
                sx={{ mt: 1 }}
              />
