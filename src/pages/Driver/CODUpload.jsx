...
const NewCOD = ({ prefill = null, fromDelivery = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const contractKey = await uploadToS3(
        contractPicture,
        'cod',
        form.customerName
      );
      const checkKey =
        form.method === 'Check' && checkPicture
      ...
      if (res.ok) {
        ...
        if (form.delivery) {
          ...
          const deliveryRes = await fetch(
            `${api}/api/delivery/${form.delivery}`,
            {
              credentials: 'include',
            }
          );
          ...
          if (deliveryRes.ok) {
            ...
            if (deliveryData?.leaseReturn?.willReturn) {
              return navigate(
                `/driver/lease-return/from-delivery/${form.delivery}`
              );
            }
          }
        }
          ...
          <Box component="form" onSubmit={handleSubmit}>
            ...
            <Box mt={3}>
              ...
              <input
                type="file"
                required
                onChange={(e) => setContractPicture(e.target.files[0])}
              />
            </Box>
            ...
            {form.method === 'Check' && (
              <Box mt={2}>
                ...
                <input
                  type="file"
                  onChange={(e) => setCheckPicture(e.target.files[0])}
                />
              </Box>
            )}
